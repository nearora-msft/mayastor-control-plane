use crate::{
    ...
    node::{
        cordon_node_reply, drain_node_reply, get_drain_reply, get_nodes_reply, get_nodes_request,
        node_grpc_client::NodeGrpcClient, uncordon_node_reply, CordonNodeRequest, DrainNodeRequest,
        GetDrainRequest, GetNodesRequest, ProbeRequest, UncordonNodeRequest,
    },
...
use common_lib::{
    ...
    types::v0::transport::{DrainState, DrainStateEnum, Filter, MessageIdVs, Node, NodeId},
};
...
impl NodeOperations for NodeClient {
    ...
    #[tracing::instrument(name = "NodeClient::get_drain", level = "debug", skip(self), err)]
    async fn get_drain(&self, id: NodeId) -> Result<DrainState, ReplyError> {
        let req = GetDrainRequest {
            node_id: id.to_string(),
        };
        // the following is a call to the autogenerated code ?
        let response = self.client().get_drain(req).await?.into_inner();
        match response.reply {
            Some(get_drain_reply) => match get_drain_reply {
                get_drain_reply::Reply::DrainState(ds) => {
                    let dss = DrainState {
                        drain_state: DrainStateEnum::try_from(ds.drain_state()).unwrap(),
                    };
                    Ok(dss)
                }
                get_drain_reply::Reply::Error(err) => Err(err.into()),
            },
            None => Err(ReplyError::invalid_response(ResourceKind::Node)),
        }
    }
